...
namespace Server.MirEnvir
{
    ...
    public class Envir
    {
        ...
        public void LoadDB()
        {
            ...
            //LoadBinaryDataIntoSQLDB();

            List<MapInfo> MapInfoList2 = SQLDataHandler.GetMapInfo();
        }

        private void LoadBinaryDataIntoSQLDB()
        {
            Stopwatch stopwatch;
            ...
            for (int i = 0; i < MapInfoList.Count; i++)
            {
                SaveMapInfoToDatabase(MapInfoList[i]);
            }
            SMain.Enqueue(string.Format("{0} MapInfo records inserted into database. Time taken: {1}", MapInfoList.Count.ToString(), stopwatch.Elapsed));
            stopwatch = null;
            ...
            for (int i = 0; i < ItemInfoList.Count; i++)
            {
                SQLDataHandler.InsertItemInfo(ItemInfoList[i]);
            }
            SMain.Enqueue(string.Format("{0} ItemInfo records inserted into database. Time taken: {1}", ItemInfoList.Count.ToString(), stopwatch.Elapsed));
            stopwatch = null;
            ...
            for (int i = 0; i < MonsterInfoList.Count; i++)
            {
                SQLDataHandler.InsertMonsterInfo(MonsterInfoList[i]);
            }
            SMain.Enqueue(string.Format("{0} MonsterInfo records inserted into database. Time taken: {1}", MonsterInfoList.Count.ToString(), stopwatch.Elapsed));
            stopwatch = null;
            ...
            for (int i = 0; i < NPCInfoList.Count; i++)
            {
                SQLDataHandler.InsertNPCInfo(NPCInfoList[i]);
            }
            SMain.Enqueue(string.Format("{0} NPCInfo records inserted into database. Time taken: {1}", NPCInfoList.Count.ToString(), stopwatch.Elapsed));
            stopwatch = null;
            ...
            for (int i = 0; i < QuestInfoList.Count; i++)
            {
                SQLDataHandler.InsertQuestInfo(QuestInfoList[i]);
            }
            SMain.Enqueue(string.Format("{0} QuestInfo records inserted into database. Time taken: {1}", QuestInfoList.Count.ToString(), stopwatch.Elapsed));
            stopwatch = null;

            //DragonInfo
            SQLDataHandler.InsertDragonInfo(DragonInfo);
            SMain.Enqueue("DragonInfo record successfully inserted into database.");

            //MagicInfo
            stopwatch = Stopwatch.StartNew();
            for (int i = 0; i < MagicInfoList.Count; i++)
            {
                SQLDataHandler.InsertMagicInfo(MagicInfoList[i]);
            }
            SMain.Enqueue(string.Format("{0} MagicInfo records inserted into database. Time taken: {1}", MagicInfoList.Count.ToString(), stopwatch.Elapsed));
            stopwatch = null;
            ...
            for (int i = 0; i < GameShopList.Count; i++)
            {
                SQLDataHandler.InsertGameShopItem(GameShopList[i]);
            }
            SMain.Enqueue(string.Format("{0} GameShopItem records inserted into database. Time taken: {1}", GameShopList.Count.ToString(), stopwatch.Elapsed));
            stopwatch = null;
            ...
            for (int i = 0; i < ConquestInfos.Count; i++)
            {
                SaveConquestInfoToDatabase(ConquestInfos[i]);
            }
            SMain.Enqueue(string.Format("{0} ConquestInfo records inserted into database. Time taken: {1}", ConquestInfos.Count.ToString(), stopwatch.Elapsed));
            stopwatch = null;
        }

        private void SaveConquestInfoToDatabase(ConquestInfo conquestInfo)
        {
            SQLDataHandler.InsertConquestInfo(conquestInfo);

            //Conquest archer guards
            for (int i = 0; i < conquestInfo.ConquestGuards.Count; i++)
            {
                SQLDataHandler.InsertConquestArcherInfo(conquestInfo.ConquestGuards[i], conquestInfo.Index);
            }

            //Conquest extra maps
            for (int i = 0; i < conquestInfo.ExtraMaps.Count; i++)
            {
                SQLDataHandler.InsertConquestExtraMapInfo(conquestInfo.Index, (int)conquestInfo.ExtraMaps[i]);
            }

            //Conquest gates
            for (int i = 0; i < conquestInfo.ConquestGates.Count; i++)
            {
                SQLDataHandler.InsertConquestGateInfo(conquestInfo.ConquestGates[i], conquestInfo.Index);
            }

            //Conquest walls
            for (int i = 0; i < conquestInfo.ConquestWalls.Count; i++)
            {
                SQLDataHandler.InsertConquestWallInfo(conquestInfo.ConquestWalls[i], conquestInfo.Index);
            }

            //Conquest sieges
            for (int i = 0; i < conquestInfo.ConquestSieges.Count; i++)
            {
                SQLDataHandler.InsertConquestSiegeInfo(conquestInfo.ConquestSieges[i], conquestInfo.Index);
            }

            //Conquest flags
            for (int i = 0; i < conquestInfo.ConquestFlags.Count; i++)
            {
                SQLDataHandler.InsertConquestFlagInfo(conquestInfo.ConquestFlags[i], conquestInfo.Index);
            }
        }

        private void SaveMapInfoToDatabase(MapInfo mapInfo)
        {
            SQLDataHandler.InsertMapInfo(mapInfo);

            for (int i = 0; i < mapInfo.Respawns.Count; i++)
            {
                SQLDataHandler.InsertRespawnInfo(mapInfo.Respawns[i], mapInfo.Index);
            }
        }
